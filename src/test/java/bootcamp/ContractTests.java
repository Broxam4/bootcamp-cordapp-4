...
public class ContractTests {
    ...
    private TokenState tokenState = new TokenState(1, alice.getParty(), bob.getParty(), 1);
    ...
    public void tokenContractRequiresZeroInputsInTheTransaction() {
        transaction(ledgerServices, tx -> {
            ...
            tx.command(Arrays.asList(alice.getPublicKey(), bob.getPublicKey()), new TokenContract.Commands.Generate());
            tx.fails();
            return null;
        });
    ...
    public void tokenContractRequiresTheTransactionsOutputToHaveAPositiveAmount() {
        TokenState zeroTokenState = new TokenState(2, alice.getParty(), bob.getParty(), 0);
        TokenState negativeTokenState = new TokenState(3, alice.getParty(), bob.getParty(), -1);
        TokenState positiveTokenState = new TokenState(4, alice.getParty(), bob.getParty(), 2);
    ...
    public void tokenContractRequiresTheIssuerToBeARequiredSignerInTheTransaction() {
        TokenState tokenStateWhereBobIsIssuer = new TokenState(5, bob.getParty(), alice.getParty(), 1);

        transaction(ledgerServices, tx -> {
            // Issuer is not a required signer, will fail.
            tx.output(TokenContract.ID, tokenState);
            tx.command(bob.getPublicKey(), new TokenContract.Commands.Generate());
            tx.fails();
            return null;
        });
        ...
        transaction(ledgerServices, tx -> {
            ...
            tx.command(alice.getPublicKey(), new TokenContract.Commands.Generate());
            tx.fails();
            return null;
        });
